  ...
  if (!session) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <p className="text-faint text-sm">Session not found.</p>
        <button onClick={() => navigate('/')} className="text-sm text-ink underline">← Home</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-bg">
      <header className="sticky top-0 z-40 bg-surface backdrop-blur border-b border-line px-4 py-3 flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="text-faint hover:text-ink transition-colors text-sm shrink-0">
          ← Back
        </button>
        <span className="text-sm font-medium font-display text-ink truncate">
      ...
      <div className="px-4 py-6 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-surface border border-line rounded-panel px-4 py-4">
          <p className="text-xs text-faint uppercase tracking-wider font-bold font-body mb-3">Summary</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-bold font-mono text-ink">{session.exercises.length}</p>
              <p className="text-xs text-faint mt-0.5">Exercises</p>
            </div>
            <div>
              <p className="text-2xl font-bold font-mono text-ink">{completedSets}</p>
              <p className="text-xs text-faint mt-0.5">Sets done</p>
            </div>
            <div>
              <p className="text-2xl font-bold font-mono text-ink">{duration ?? '—'}</p>
              <p className="text-xs text-faint mt-0.5">Duration</p>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-line flex justify-between text-xs text-faint">
        ...
        {session.exercises.map(ex => {
          ...
          return (
            <div key={ex.id} className="bg-surface border border-line rounded-panel overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-line">
                <span className="font-semibold text-ink text-sm">{ex.exerciseName}</span>
                <span className="text-xs font-mono text-faint">{completedSets}/{ex.sets.length} sets</span>
              </div>

              <div className="px-4 py-3 space-y-1.5">
                {ex.sets.map((set, i) => (
                  <div key={set.id} className={`flex items-center gap-3 text-sm ${!set.completed ? 'opacity-40' : ''}`}>
                    <span className="w-5 text-center text-xs font-mono text-faint shrink-0">{i + 1}</span>
                    <span className="flex-1 text-ink font-mono">
                    ...
                    {set.completed && (
                      <span className="w-5 h-5 rounded-full bg-done flex items-center justify-center shrink-0">
        ...
        {totalSets === 0 && (
          <p className="text-center text-faint text-sm py-8">No sets recorded.</p>
        )}
        ...
        {session.exercises.length > 0 && (
          <button
            ...
            className="w-full py-4 rounded-2xl bg-brand hover:opacity-90 active:scale-[0.98] transition text-brand-ink font-semibold text-base"
          >
            Repeat this workout
          </button>
        )}
